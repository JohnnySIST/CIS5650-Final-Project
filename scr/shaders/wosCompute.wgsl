...
// REGULAR WoS
fn distanceToBoundaryWoS(pos: vec2f, texSize: vec2u) -> vec2f {
...
fn walkOnSpheres(startPos: vec2f, texSize: vec2u, rngState: ptr<function, u32>) -> f32 {
  ...
  for (var step = 0; step < maxSteps; step++) {
    let boundaryResult = distanceToBoundaryWoS(pos, texSize);
...
fn main(@builtin(global_invocation_id) id: vec3u) {
  ...
  let numWalks = 4u;
  var totalTemp = 0.0;
  var seed = u32(coords.x) * 747796405u + u32(coords.y) * 2891336453u * totalWalks;
  
  for (var i = 0u; i < numWalks; i++) {
    let temp = walkOnSpheres(worldPos, texSize, &seed);//walkOnSpheres(worldPos, texSize, &seed);
    totalTemp += temp;
  }
