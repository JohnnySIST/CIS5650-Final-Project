...
export const example_circles: Circle[] = (
  ...
  ] satisfies Omit<Circle, "boundary_type">[]
).map((circle) => ({ ...circle, boundary_type: BoundaryType.DIRICHILET }));

export const example_segments: Segment[] = (
  [
    { start: [-1, -1], end: [1, 1], widthRadius: 0.05, boundary_value: 0.5 },
    { start: [1, -1], end: [-1, 1], widthRadius: 0.05, boundary_value: 0.5 },
  ] satisfies Omit<Segment, "boundary_type">[]
).map((segment) => ({ ...segment, boundary_type: BoundaryType.NEUMANN }));
